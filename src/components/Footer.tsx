...
export default function Footer() {
  return (
    <footer className="bg-primary-800 text-white py-14 px-6">
      <div className="mx-auto max-w-4xl text-center">
        ...
        <p className="text-white/70 text-sm mb-8">
          Swim. Bike. Run. <span className="text-secondary-400 font-semibold">Thrive.</span>
        </p>
        ...
        <div className="flex justify-center gap-3 mb-10">
          {[
            ...
            <span
              key={i}
              className="w-9 h-9 rounded-full bg-white/10 flex items-center justify-center text-white/50 hover:bg-secondary-600 hover:text-white transition-colors cursor-pointer"
            >
              <svg width="15" height="15" fill="currentColor" viewBox="0 0 24 24">{icon}</svg>
            </span>
          ))}
        </div>

        <div className="border-t border-white/10 pt-6 flex flex-col sm:flex-row justify-between items-center gap-3 text-[11px] text-white/30">
