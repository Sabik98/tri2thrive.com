...
export default function Founders() {
  return (
    <section id="founders" className="py-20 md:py-28 bg-neutral-50">
        ...
        <div className="grid md:grid-cols-2 gap-12 max-w-3xl mx-auto">
          {founders.map((founder) => (
            <div key={founder.name} className="text-center">
              {/* Circular photo */}
              <div className="w-40 h-40 md:w-48 md:h-48 rounded-full overflow-hidden mx-auto mb-6">
                <Image
                  src={founder.photo}
                  alt={founder.name}
                  width={300}
              ...
              <div className="mt-4 flex justify-center gap-3">
                {["Facebook", "Instagram"].map((platform) => (
                  <span
                    key={platform}
                    className="w-9 h-9 rounded-full bg-neutral-100 flex items-center justify-center text-neutral-300 hover:bg-secondary-600 hover:text-white transition-colors cursor-pointer"
                  >
